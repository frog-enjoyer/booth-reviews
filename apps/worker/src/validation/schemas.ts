import {
  MAX_BATCH_SUMMARY_ITEMS,
  MAX_REVIEW_BODY_LENGTH,
  MIN_NEGATIVE_REVIEW_BODY_LENGTH,
} from '@booth-addon/shared';
import { z } from 'zod';

import { reportReasons } from '../moderation/policy';
...
const reviewFieldsSchema = z.object({
  ...
  purchaseState: z
    .enum(['unknown', 'not_detected', 'appears_purchased'])
    .default('unknown'),
});

function hasValidNegativeBody(value: {
  rating: 'up' | 'down';
  body: string;
}): boolean {
  ...
  return (
    trimmed.length === 0 ||
    value.rating !== 'down' ||
    trimmed.length >= MIN_NEGATIVE_REVIEW_BODY_LENGTH
  );
}
...
export const reviewUpdateSchema = reviewFieldsSchema.refine(
  hasValidNegativeBody,
  {
    message: 'Negative reviews need at least 20 characters',
    path: ['body'],
  },
